...
interface Article { id: string; title: string; category: string; published: boolean; createdAt: string; author: string; summary?: string; content?: string; slug?: string; }
...
export function AdminArticles() {
  ...
  const [form, setForm] = useState({ title: '', category: '假牙百科', summary: '', content: '', author: 'Novadent 編輯部', slug: '', published: false });
  const [submitting, setSubmitting] = useState(false);

  const load = () => {
    setLoading(true);
    apiFetch('/articles?limit=100&all=true')
      .then(r => r.json())
  ...
  const openCreate = () => { setEditTarget(null); setForm({ title: '', category: '假牙百科', summary: '', content: '', author: 'Novadent 編輯部', slug: '', published: false }); setShowModal(true); };
  const openEdit = (a: Article) => { setEditTarget(a); setForm({ title: a.title, category: a.category, summary: a.summary || '', content: a.content || '', author: a.author, slug: a.slug || '', published: a.published }); setShowModal(true); };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const url = editTarget ? `/articles/${editTarget.id}` : '/articles';
    const res = await apiFetch(url, { method: editTarget ? 'PUT' : 'POST', body: JSON.stringify({ ...form, slug }) });
    setSubmitting(false);
  ...
  const handleTogglePublish = async (a: Article) => {
    await apiFetch(`/articles/${a.id}`, { method: 'PUT', body: JSON.stringify({ published: !a.published }) });
    load();
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[600px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>{['標題', '分類', '作者', '狀態', '操作'].map(h => <th key={h} className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">{h}</th>)}</tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? <tr><td colSpan={5} className="text-center py-12 text-slate-400">載入中...</td></tr>
              : articles.length === 0 ? <tr><td colSpan={5} className="text-center py-12 text-slate-400">尚無文章</td></tr>
              : articles.map(a => (
